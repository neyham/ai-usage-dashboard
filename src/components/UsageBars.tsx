...
export function UsageBars({
  ...
  if (balance) {
    return (
      <div className="ip-bars" data-kind="balance">
  ...
  if (windows.length === 0) {
    return (
      <div className="ip-bars" data-empty="true">
        <span className="usage-unavailable">USAGE DATA UNAVAILABLE</span>
      </div>
    );
  }

  const peak = Math.max(...windows.map((win) => win.percent));
  const peakTone = toneOf(peak);
  return (
    <div className="ip-bars" role="group" data-tone={peakTone}>
      <div
